...
- (void)start:(bnb::camera::push_frame_cb_t)delegate;
- (void)stop;
@end
...
@property bnb::camera::push_frame_cb_t callback;
...
- (std::vector<bnb::camera::camera_device_description>)getConnectedDevices;
...
- (std::vector<bnb::camera::camera_device_description>)getConnectedDevices
{
    std::vector<bnb::camera::camera_device_description> devices;
    devices.reserve([self.videoDevices count]);
    for (AVCaptureDevice* obj in self.videoDevices) {
        bnb::camera::camera_device_description descr;
...
bnb::camera::camera(bnb::camera::push_frame_cb_t cb)
    : m_push_frame_cb(cb)
...
void bnb::camera::set_device_by_index(uint32_t index)
{
    m_device_index = index;
    [m_impl->wrapped switchCamera:index];
}

void bnb::camera::set_device_by_id(const std::string& device_id)
{
...
{
    auto cam_ptr = std::make_shared<bnb::camera>(cb);
...
const std::vector<bnb::camera::camera_device_description> bnb::camera::get_connected_devices() const
{
    return m_connected_devices;
}

/* camera::get_current_device_index */
size_t bnb::camera::get_current_device_index() const
{
